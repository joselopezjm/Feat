					...
					synchronized (Lowrequests) {
						...
						if (!Lowrequests.isEmpty()) {
							...
							if (count >= 10 && count < 12) {
								count = 0;
								try {
									 System.out.println("WS LOWREQUESTS.size()"+Lowrequests.size());
//									 if(Highrequests.isEmpty()){
									...
									if (obj.length < 3) {
										...
										if ((obj[1].getClass().getSimpleName()).equals("WsSession")) {
											System.out.println("WS LOW THREAD EQUALS WsSession");
											...
											if (Highrequests.isEmpty()) {
												if (autoregulate) {
													if (times.size() < 5) {
														...
														timeout = (times.get(0) + times.get(1) + times.get(2)
																+ times.get(3) + (end - start)) / 5;
													}
													times.remove(0);
													times.add((end - start));
												}
			...
			public void run() {
				while (true) {
					synchronized (Highrequests) {
						if (!Highrequests.isEmpty()) {
							 System.out.println("WS HIGHREQUESTS.size()"+Lowrequests.size());
							try {
		...
		while (true) {
			synchronized (HighPriority) {
				if (!HighPriority.isEmpty()) {
					System.out.println("Highpriority Vector");
			...
			synchronized (LowPriority) {
				if (!LowPriority.isEmpty()) {
					System.out.println("lowpriority Vector");
	...
	public String getLowResponse(Long[] id) {
		if (id[0] == (long) 0) {
			System.out.println(" LOW RESPONSE if (id[0] == (long) 0)");
			...
			Long[] newid = { (long) 3, id[1] };
			System.out.println("LOW RESPONSE BEFORE	 while (LowResponses.size() < 1) {");

			while (LowResponses.size() < 1) {
			}
			System.out.println("LOW RESPONSE AFTER while (LowResponses.size() < 1) {");
			return getLowResponse(newid);
		}

		if (id[0] == (long) 3) {
//			if(LowResponses.size()<1){
//				return getLowResponse(id);
//			}
			System.out.println("LOW RESPONSE if (id[0] == (long) 3) {");
			for (int i = 0; i < LowResponses.size(); i++) {
			...
			System.out.println("LOW RESPONSE if (id[0] == (long) 3) { BEFORE WHILE");

			while (LowResponses.size() < 1) {
			}
			System.out.println("LOW RESPONSE if (id[0] == (long) 3) { AFTER WHILE");
			return getLowResponse(id);
		}
		return response;
	}
	...
	public synchronized boolean isJson(String x) {
		try {
				...
				if (params.size() == 0) {
					...
					if ((boolean) security[1]) {
						...
						try {
							...
							Boolean flag = false;
							 try {
							    	JsonElement jelement1 = new JsonParser().parse(x);
							        flag= true;
					...
					if ((boolean) security[1]) {
						...
						try {
							...
							Boolean flag = false;
							 try {
							    	JsonElement jelement1 = new JsonParser().parse(x);
//									 System.out.println(jelement1.isJsonNull());
//							    	if(jelement1.isJsonNull()){
	...
	public synchronized void addtoLST(JsonObject request) {
		...
		blockingQueueLST.add(new Runnable() {
			...
			public void run() {
				...
				if (params.size() == 1) {
					System.out.println("ADDTOLST if (params.size() == 1");
					SecurityComponent check = new SecurityComponent();
					Object[] security = check.CheckSecurityData(iduser, className, methName);
					if ((boolean) security[1]) {
						// Setup reflection call
